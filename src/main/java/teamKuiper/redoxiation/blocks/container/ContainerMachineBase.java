package teamKuiper.redoxiation.blocks.container;
...
public class ContainerMachineBase extends Container {
	...
	private TileMachineBase machineBase;
	...
	public final int FURNACE_SLOTS_COUNT;

	// slot index is the unique index for all slots in this container i.e. 0 - 35
	// for invPlayer then 36 - 49 for tileBlastFurnace
	private final int VANILLA_FIRST_SLOT_INDEX = 0;
	private final int FIRST_FUEL_SLOT_INDEX = VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT;
	private final int FIRST_INPUT_SLOT_INDEX;
	private final int FIRST_OUTPUT_SLOT_INDEX;

	// slot number is the slot number within each component; i.e. invPlayer slots 0
	// - 35, and tileBlastFurnace slots 0 - 14
	private final int FIRST_FUEL_SLOT_NUMBER = 0;
	private final int FIRST_INPUT_SLOT_NUMBER;
	private final int FIRST_OUTPUT_SLOT_NUMBER;

	public ContainerMachineBase(InventoryPlayer invPlayer, TileMachineBase machineBase) {
		this.machineBase = machineBase;

		int[] slotsCount = machineBase.getSlotCount();

		FUEL_SLOTS_COUNT = slotsCount[0];
		INPUT_SLOTS_COUNT = slotsCount[1];
		OUTPUT_SLOTS_COUNT = slotsCount[2];
		FURNACE_SLOTS_COUNT = FUEL_SLOTS_COUNT + INPUT_SLOTS_COUNT + OUTPUT_SLOTS_COUNT;

		FIRST_INPUT_SLOT_INDEX = FIRST_FUEL_SLOT_INDEX + FUEL_SLOTS_COUNT;
		FIRST_OUTPUT_SLOT_INDEX = FIRST_INPUT_SLOT_INDEX + INPUT_SLOTS_COUNT;

		FIRST_INPUT_SLOT_NUMBER = FIRST_FUEL_SLOT_NUMBER + FUEL_SLOTS_COUNT;
		FIRST_OUTPUT_SLOT_NUMBER = FIRST_INPUT_SLOT_NUMBER + INPUT_SLOTS_COUNT;
		...
		for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
			...
			addSlotToContainer(
					new SlotFuel(machineBase, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
		}
		...
		for (int x = 0; x < INPUT_SLOTS_COUNT; x++) {
			...
			addSlotToContainer(new SlotSmeltableInput(machineBase, slotNumber, INPUT_SLOTS_XPOS + SLOT_X_SPACING * x,
					INPUT_SLOTS_YPOS));
		}
		...
		for (int x = 0; x < OUTPUT_SLOTS_COUNT; x++) {
			...
			addSlotToContainer(new SlotOutput(machineBase, slotNumber, OUTPUT_SLOTS_XPOS + 26 * x, OUTPUT_SLOTS_YPOS));
		}
	}
	...
	public boolean canInteractWith(EntityPlayer player) {
		return machineBase.isUseableByPlayer(player);
	}
	...
	// returns null if the source slot is empty, or if none of the source slot items
	// could be moved.
	...
	// in sendProgressBarUpdate with the value in
	// updateProgressBar()
	...
	public void detectAndSendChanges() {
		...
		boolean fieldHasChanged[] = new boolean[machineBase.getFieldCount()];
		if (cachedFields == null) {
			cachedFields = new int[machineBase.getFieldCount()];
			allFieldsHaveChanged = true;
		}
		for (int i = 0; i < cachedFields.length; ++i) {
			if (allFieldsHaveChanged || cachedFields[i] != machineBase.getField(i)) {
				cachedFields[i] = machineBase.getField(i);
				fieldHasChanged[i] = true;
			}
		}

		// go through the list of crafters (players using this container) and update
		// them if necessary
		for (int i = 0; i < this.crafters.size(); ++i) {
			ICrafting icrafting = (ICrafting) this.crafters.get(i);
			for (int fieldID = 0; fieldID < machineBase.getFieldCount(); ++fieldID) {
	...
	// values given to sendProgressBarUpdate. In this case we are using fields so we
	...
	public class SlotFuel extends Slot {
		...
		public boolean isItemValid(ItemStack stack) {
			return machineBase.isItemValidForFuelSlot(stack);
		}
	}
	...
	public class SlotSmeltableInput extends Slot {
		...
		@Override
		public boolean isItemValid(ItemStack stack) {
			return machineBase.isItemValidForInputSlot(stack);
		}
	}
